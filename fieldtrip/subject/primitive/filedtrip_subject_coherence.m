function coherence_sub = filedtrip_subject_coherence(input, varargin)

coherence_sub = [];


input_file_name = input.segment.input_file_name;
ntrials = input.segment.ntrial;
triallength = input.segment.triallength;

freq_foilim = input.freq.foilim; % limits of the frequencies of interest



%% import data from eeglab to fieldtrip and segment them into pieces of selected duration 
cfg = [];
cfg.dataset              = input_file_name;
cfg.trialfun             = 'ft_trialfun_general';
cfg.trialdef.triallength = triallength;                      % duration in seconds
cfg.trialdef.ntrials     = ntrials; % inf;                    % number of trials, inf results in as many as possible
cfg                      = ft_definetrial(cfg);

data_segmented           = ft_preprocessing(cfg);
ch_labels = data_segmented.label;
comb_ch = combnk(ch_labels,2);
...
cfg.taper =  'dpss';
cfg.tapsmofrq = 1;
cfg.foilim     = freq_foilim;%[0 12];
cfg.output = 'fourier';
cfg.keep_trials = 'yes';
data_freq = ft_freqanalysis(cfg, data_segmented);

%% compute coherence between all channels and for all frequencies of interest
cfg = [];
cfg.method = 'coh';
coherence_sub = ft_connectivityanalysis(cfg, data_freq);
