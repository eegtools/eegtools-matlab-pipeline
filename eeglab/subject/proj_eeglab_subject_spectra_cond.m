function spectra = proj_eeglab_subject_spectra_cond(project, varargin)

spectra = [];

freqrange       = project.subject_spectra_cond.freqrange;
freq            = project.subject_spectra_cond.freq;
plotchans       = project.subject_spectra_cond.plotchans;
% band_analysis    = project.subject_spectra_cond.band_analysis;
% chan_analysis    = project.subject_spectra_cond.chan_analysis;
scale            = project.subject_spectra_cond.scale;


list_band_lim =  project.subject_spectra_cond.band_analysis.lim;   %= [0, 2; 2, 4; 4, 8; 8, 12];
list_band_name = project.subject_spectra_cond.band_analysis.name;  % = {'delta1', 'delta2', 'theta', 'alpha1', 'alpha2'};

list_roi_ch =  project.subject_spectra_cond.roi_analysis.ch;%     =  {{'O1','O2'},{'T7','T8'}, {'C3', 'C4'}};
list_roi_name = project.subject_spectra_cond.roi_analysis.name;   %=  {'Visual','Auditory', 'Tactile'};

list_allroi_ch = [project.subject_spectra_cond.roi_analysis.ch{:}];

list_agebin_lim  = project.subject_spectra_cond.agebin.lim;
list_agebin_name = project.subject_spectra_cond.agebin.name;

tab= length(list_agebin_name);

set_caxis = project.subject_spectra_cond.band_analysis.caxis;
plot_single_subject = project.subject_spectra_cond.plot_single_subject;
...
subject_spectra_cond_path = fullfile(results_path,['subject_spectra_cond','_',str,'_',scale]);
if not(exist(subject_spectra_cond_path))
    mkdir(subject_spectra_cond_path)
end

plot_dir = fullfile(subject_spectra_cond_path,'plot');
if not(exist(plot_dir))
    mkdir(plot_dir)
end

plot_dir_group = fullfile(plot_dir,['group','_',project.subject_spectra_cond.analysis_name]);
...
fprintf(fid1,'%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n','group','sub','sex','age_y','cond','duration','roi','band','max_abs','max_rel','f_max');
fprintf(fid2,'%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n','group','sub','sex','age_y','cond','duration','roi','freq','abs','rel','log');
fprintf(fid3,'%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n','group','sub','sex','age_y','cond','duration','ch','band','max_abs','max_rel','f_max');
fprintf(fid4,'%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n','group','sub','sex','age_y','cond','duration','ch','freq','abs','rel','log');

allcond = ['baseline',project.segmenting.event.list_label];
tcond = length(allcond);

input_spectra.ylim_rel = project.subject_spectra_cond.ylim_rel;
input_spectra.ylim_abs = project.subject_spectra_cond.ylim_abs;
...
for subj=1:numsubj
    ...
    for ncond = 1:tcond
        ...
        input_spectra.plot_label     = [subj_name, '_',groupsub,'_' ,num2str(agesub),'_',cond];
        
        input_spectra.input_file_name   = proj_eeglab_subject_get_filename(project, subj_name, get_filename_step, 'custom_suffix', custom_suffix, 'custom_input_folder', custom_input_folder,'cond_name',cond);
        if exist(input_spectra.input_file_name)
            ...
            %         [names{subj},ranks{subj},ica_types{subj},durations{subj}, EEG]         = eeglab_subject_spectra_cond(inputfile, project.paths.output_preprocessing, project.eegdata.eeg_channels_list, project.import.reference_channels, ica_type,do_pca,ica_sr);
            %            spectra_sub         = eeglab_subject_spectra_cond(inputfile, project.paths.output_preprocessing, project.eegdata.eeg_channels_list, project.import.reference_channels, ica_type,do_pca,ica_sr);
            ...
            if strcmp(cond,'baseline' )
                spectra_sub_baseline =  spectra_sub;
            end
            plot_label = ['spectra_',subj_name,'_',cond ];
            fname_outputmat = [plot_label '.mat'];
            ...
            if not(strcmp(cond, 'baseline'))
                
                for nroi  = 1:length(list_roi_name)
                    if strcmp(plot_single_subject,'on')
                        
                        roispectra_log = spectra_sub.roi(nroi).roispectra_log;
                        roispectra_log_baseline = spectra_sub_baseline.roi(nroi).roispectra_log;
                        
                        roispectra_log_desync =  roispectra_log - roispectra_log_baseline;
                        ...
                        plot(spectra_sub.freqs,roispectra_log_desync,'LineWidth',2);
                        xlabel('Frequency (Hz)')
                        ...
                        plot_dir_ch = fullfile(plot_dir,'roi_sp_desync');
                ...
                for nf = 1:tf
                    ...
                    current_freq = freq(nf);
                    sel_data = spectra_sub.freqs == current_freq;
                    vdata = spectra_sub.log_spectra(:,sel_data) - spectra_sub_baseline.log_spectra(:,sel_data) ;
                    ...
                    topoplot(vdata,spectra_sub.chanlocs);
                ...
                suptitle(plot_label_ch);
                
                plot_dir_ch = fullfile(plot_dir,'topo_sp_desync');
                if not(exist(plot_dir_ch))
                    mkdir(plot_dir_ch)
                end
                ...
                inputsf.fig         = fig;
                inputsf.name_embed  = 'subject_spectra_ch';
                inputsf.suffix_plot = ['subject_spectra_ch',plot_label];
                save_figures(inputsf,'res','-r100');
                ...
                plot_dir_ch = fullfile(plot_dir,'topo_sp_z_desync');
                if not(exist(plot_dir_ch))
                    mkdir(plot_dir_ch)
                end
            ...
            for nroi = 1:troi
                ...
                for nf = 1:length(spectra_sub.freqs)
                    ...
                    fprintf(fid2,...
                        ...
                        spectra_sub.roi(nroi).roispectra_log(nf)...
                        );
            ...
            for nch = 1:tch
                ...
                for nf = 1:length(spectra_sub.freqs)
                    ...
                    fprintf(fid4,...
                        ...
                        spectra_sub.ch(nch).chspectra_log(nf)...
                        );
...
if (strcmp(project.subject_spectra_cond.do_group,'on'))
