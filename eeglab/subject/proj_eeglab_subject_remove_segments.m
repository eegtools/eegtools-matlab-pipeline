...
%
%%
function EEG = proj_eeglab_subject_remove_segments(project, varargin)
...
%     ica_types ={};
    ...
    for subj=1:numsubj
        ...
        if isempty(segm2remove)
            EEG = [];
            disp(['not removing any segment ','from',  inputfile])
        else
            disp(['removing bad segments from',  inputfile])
            EEG         = eeglab_subject_remove_segments(inputfile,segm2remove);
        end
