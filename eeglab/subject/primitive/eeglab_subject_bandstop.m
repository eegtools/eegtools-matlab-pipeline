%% function  EEG = proj_eeglab_subject_bandstop(EEG,params)
% apply a stopband (notch) filter based on the parameters provided by the structure params:
% params.filter_algorithm                 
% params.channels_list               
% params.notch_fcenter
% params.notch_fspan
%
function  EEG = eeglab_subject_bandstop(EEG, params)

%  andrà applicato un ciclo for di notch che spazzolino tutte le armoniche
%  bisognerà poter selezionare tra passabanda e tagliabanda
    if nargin < 2
   ...
   for nharmonics = 1:length(notch_fcenter) 
       ...
       %##################################################################################
            ...
            case 'causal_pop_iirfilt_12'
                ...
                EEG2                          = pop_iirfilt(EEG2,notch_ff1(nharmonics), notch_ff2, [], 1, 1);
                EEG.data(params.channels_list,:) = EEG2.data;

            case 'noncausal_pop_iirfilt_12'
                EEG2                          = pop_select(EEG,'channel',channels_list);
                EEG2                          = pop_iirfilt(EEG2,notch_ff1(nharmonics), notch_ff2(nharmonics), [], 1, 0);
                EEG.data(params.channels_list,:) = EEG2.data;
        ...
        %##################################################################################
            ...
            case 'causal_pop_eegfilt_12'
                EEG2                          = pop_select(EEG,'channel',params.channels_list);
                EEG2                          = pop_eegfilt( EEG2,notch_ff1(nharmonics), notch_ff2(nharmonics), [], 1, 0, 0, 'fir1', 1);
                EEG.data(params.channels_list,:) = EEG2.data;

            case 'noncausal_pop_eegfilt_12'
                EEG2                          = pop_select( EEG,'channel',params.channels_list);
                EEG2                          = pop_eegfilt( EEG2,notch_ff1(nharmonics), notch_ff2(nharmonics), [], 1, 0, 0, 'fir1', 0);
                EEG.data(params.channels_list,:) = EEG2.data;
        ...
        %##################################################################################
           ...
           case 'pop_eegfilt'
               EEG2                          = pop_select(EEG,'channel',params.channels_list);
               EEG2                          = pop_eegfilt( EEG2,notch_ff1(nharmonics), notch_ff2(nharmonics), [], 1);
               EEG.data(params.channels_list,:) = EEG2.data;
        ...
        %##################################################################################        
            ...
            case 'causal_pop_eegfiltnew_13'
                EEG2                          = pop_select(EEG,'channel',params.channels_list);
                EEG2                          = pop_eegfiltnew( EEG2,notch_ff1(nharmonics), notch_ff2(nharmonics),[] ,1,false,0,true);
                EEG.data(params.channels_list,:) = EEG2.data;
            %  non casual zero phase
            case 'noncausal_pop_eegfiltnew_13'
                EEG2                          = pop_select(EEG,'channel',params.channels_list);
                EEG2                          = pop_eegfiltnew(EEG2,notch_ff1(nharmonics), notch_ff2(nharmonics),[] ,1,false,0,false);
                EEG.data(params.channels_list,:) = EEG2.data;
...
%                EEG = pop_basicfilter( EEG, params.channels_list, notch_fcenter(nharmonics), notch_fcenter(nharmonics), 2, 'notch', 0, [] );
                EEG = pop_basicfilter( EEG, params.channels_list, ...
                    'Boundary', 'boundary',...
                    'Cutoff', notch_fcenter(nharmonics),....
                    'Design', 'notch',...
                    'Filter', 'PMnotch'...
                    ...'Order', 190 ...
                    );
