function OUTEEG = eeglab_subject_interpolate_channels(input, varargin)
...
template_file = input.eeglab_channels_file_path;

select_montage = find(ismember(montage_names,acquisition_system));
...
if not(isempty(EEG_poly))
    ...
    for nchpoly = 1:tchpoly
        ...
        if not(isempty(OUTEEG.chanlocs))
             equalfields = length(fieldnames(OUTEEG.chanlocs)) == length(fieldnames(EEG_poly.chanlocs));
           
            if not(equalfields)
                for nc = 1:EEG_poly.nbchan
                    EEG_poly.chanlocs(nc).sph_theta_besa = [];
                    EEG_poly.chanlocs(nc).sph_phi_besa   = [];
                end
                
                for nc = 1:OUTEEG.nbchan
                    OUTEEG.chanlocs(nc).sph_theta_besa = [];
                    OUTEEG.chanlocs(nc).sph_phi_besa   = [];
                end
            end
