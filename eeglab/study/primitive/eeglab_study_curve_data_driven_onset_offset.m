...
%         output.ch{nf_gf,nch} = fcoo;
    ...
    for nch = 1: tot_ch_def
        ...
        ind_ch = sel_ch(nch);
        data_ch =  sigmat_pruned_0(ind_ch,:);
        if sum(data_ch) == tot_t
            ...
            for n_ini = 1:length(ini_tw)
                mat_tw(n_ini,:) = [ini_tw(n_ini), fin_tw(n_ini)];
            end            
        end
    ...
    for nch = 1:tot_ch
        ...
        for ntw = 1:tot_tw;
            ...
            output_tw(ntw).barycenter                                                   = nan;                                        % baricentro (tempo pesato sulle ampiezze) curva tra onset e offset
                        ...
                        tmax_deflection                                                            = times(curve_ch == max_deflection);
                        min_deflection                                                             = min(curve_ch(seltsig));
                        tmin_deflection                                                            = times(curve_ch == min_deflection);
                        ...
                        barycenter                                                                 = sum( times(selt2).*curve_ch(selt2)) / sum(curve_ch(selt2));
