% std_stat_corr() - compute statistics for ERP/spectral traces or ERSP/ITC images
...
%           in the three groups. See below and >> help statcond_corr
...
%  statcond_corr     - [cell] condition statistic values (F or T).
...
function [pcond, pgroup, pinter, statscond, statsgroup, statsinter] = std_stat_corr(data,num_tails, varargin)
...
if strcmpi(opt.mode, 'eeglab')
    ...
    if strcmpi(opt.condstats, 'on') && nc > 1
        for g = 1:ng
            [F df pval] = statcond_corr(data(:,g), num_tails,'method', opt.eeglab.method, 'naccu', opt.eeglab.naccu, 'paired', opt.paired{1});
    ...
    if strcmpi(opt.groupstats, 'on') && ng > 1
        for c = 1:nc
            [F df pval] = statcond_corr(data(c,:), num_tails,'method', opt.eeglab.method, 'naccu', opt.eeglab.naccu, 'paired', opt.paired{2});
    ...
    if ( strcmpi(opt.groupstats, 'on') && strcmpi(opt.condstats, 'on') ) & ng > 1 & nc > 1
        ...
        [F df pval] = statcond_corr(data, num_tails,'method', opt.eeglab.method, 'naccu', opt.eeglab.naccu, 'paired', opt.paired{1});
    ...
    if strcmpi(opt.condstats, 'on') && nc > 1
        for g = 1:ng
            [F df pval]  = statcond_corrfieldtrip(data(:,g), 'paired', opt.paired{1}, params{:});
    ...
    if strcmpi(opt.groupstats, 'on') && ng > 1
        for c = 1:nc
            [F df pval]   = statcond_corrfieldtrip(data(c,:), 'paired', opt.paired{2}, params{:});
    ...
    if ( strcmpi(opt.groupstats, 'on') && strcmpi(opt.condstats, 'on') ) & ng > 1 & nc > 1
        opt.paired  = sort(opt.paired); % put 'off' first if present
        [F df pval] = statcond_corrfieldtrip(data, 'paired', opt.paired{1}, params{:});
        for index = 1:length(pval)
