...
for nroi = 1:length(roi_list)
    ...
    if(isempty(levels_f2))
        if strcmp(ersp_measure, 'Pfu')
            for nf1=1:length(levels_f1)
                ersp{nf1,1}=(10.^(ersp{nf1,nf2}/10)-1)*100;
            end
        end
        ...
        for nf1=1:length(levels_f1)
            if ~isempty(list_select_subjects)
                vec_select_subjects=ismember(original_list_design_subjects{nf1,1},list_select_subjects);
                ersp{nf1,1}=ersp{nf1,1}(:,:,vec_select_subjects);
            end
        end
        ...
        ersp_roi=[];
        for nf1=1:length(levels_f1)
            if (length(roi_channels)  > 1)
                ersp_roi{nf1,1}=squeeze(mean(ersp{nf1,1},3));
            else
                ersp_roi{nf1,1}=ersp{nf1,1};
            end
        end
    else
        if strcmp(ersp_measure, 'Pfu')
    ...
    if isempty(levels_f2)
        for nband=1:length(frequency_bands_list)
            ...
            for nf1=1:length(levels_f1)
                ...
                ersp_matrix_sub                                         = ersp_roi{nf1,1}(:,:,nsub);
                ...
                ersp_curve_roi_fb{nf1,1}(:,nsub)=mean(ersp_roi{nf1,1}(sel_freqs,:,nsub),1);
                end
            end
...
for nroi = 1:length(roi_names)
    ...
    for nband=1:length(frequency_bands_list)
        ...
        if isempty(levels_f2)
            for nf1=1:length(levels_f1)
                ersp_topo_tw_fb_roi_avg{nf1,1}=ersp_curve_roi_fb_stat.dataroi(nroi).databand(nband).ersp_curve_roi_fb{nf1,1};
            end
        else
            for nf1=1:length(levels_f1)
                for nf2=1:length(levels_f2)
