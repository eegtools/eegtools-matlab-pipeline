% std_erpplot_corr() - Command line function to plot STUDY cluster component ERPs. Either 
...
%                 and std_propplot().
% Usage:    
%   >> [STUDY] = std_erpplot_corr(STUDY, ALLEEG, key1, val1, key2, val2);  
%   >> [STUDY erpdata erptimes pgroup pcond pinter] = std_erpplot_corr(STUDY, ALLEEG, ...);  
%
% Inputs:
...
%
%   Example:
%            >> [STUDY] = std_erpplot_corr(STUDY,ALLEEG, 'clusters', 2, 'comps', 'all');
...
function [STUDY, erpdata, alltimes, pgroup, pcond, pinter] = std_erpplot_corr(STUDY, ALLEEG, varargin)

if nargin < 2
    help std_erpplot_corr;
    return;
end;
erpdata = []; alltimes = []; 
pgroup = []; pcond = []; pinter = [];
...
opt = finputcheck( options, ...
                             { 'design'      'integer' []              STUDY.currentdesign;
                               ...
                               'subject'     'string'  []              '' }, 'std_erpplot_corr');
...
set(gcf, 'Visible', 'off')
    ...
    if ~isempty(params.topotime) && all(~isnan(params.topotime))
        std_chantopo(erpdata, 'groupstats', pgroup, 'condstats', pcond, 'interstats', pinter, 'caxis', params.ylim, ...
                                      'chanlocs', locs, 'threshold', alpha, 'titles', alltitles);set(gcf, 'Visible', 'off')
    else
    ...
    set(gcf,'name',['Channel ' datatypestr ]);set(gcf, 'Visible', 'off')
    axcopy(gca);set(gcf, 'Visible', 'off')
else 
    % plot component
    % --------------
    if length(opt.clusters) > 1, figure('color', 'w');set(gcf, 'Visible', 'off'); end;
    ...
    for index = 1:length(opt.clusters)
        ...
        if length(opt.clusters) > 1, subplot(nr,nc,index); end;set(gcf, 'Visible', 'off')
        if strcmpi(opt.datatype, 'erp')
        ...
        std_plotcurve(alltimes, erpdata, 'condnames', allconditions, 'legend', alllegends, 'groupnames', allgroups, 'plotstderr', opt.plotstderr, ...
                                          ...
                                          'plotsubjects', opt.plotsubjects, plotcurveopt{:});set(gcf, 'Visible', 'off');
    end;
    
    set(gcf,'name', ['Component ' datatypestr ] );set(gcf, 'Visible', 'off')
    axcopy(gca);set(gcf, 'Visible', 'off')
end;
