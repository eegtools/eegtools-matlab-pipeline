...
for c = 1:nc
    for g = 1:ng
        ...
        if ~isempty(data{c,g})
            ...
            tftopo_tw_fb( tmpplot, times, freqs, 'events', tmpevents, 'title', opt.titles{c,g}, options{:});set(gcf, 'Visible', 'off');
...
if ~isempty(opt.groupstats) && ~isempty(opt.condstats) && ng > 1 && nc > 1
    ...
    tftopo_tw_fb( pinterplot,  times, freqs, 'title', opt.titles{nc+1,ng+1}, options{:});set(gcf, 'Visible', 'off');
    caxis([-maxplot maxplot]);
    %     ylabel('');
    xlabel('Time window','FontSize',12);
    ylabel('Frequency band','FontSize',12);
