% std_plotcurve_erp() - plot ERP or spectral traces for a STUDY component 
%                   or channel cluster 
% Usage:
    ...
    for g = 1:ngplot
        ...
        if ~isempty(data{c,g})
            ...
            elseif iscell(tmpdata)
                plotcurve( allx, tmpdata{1}, 'colors', tmpcol, 'maskarray', tmpdata{2}, plotopt{3:end}, 'title', opt.titles{c,g});set(gcf, 'Visible', 'off')
            else
...
%                     
                ...
                if ~strcmpi(opt.plotstderr, 'off') 
                    if ~dimreduced_sizediffers
                        ...
                        set(gcf, 'renderer', 'OpenGL');set(gcf, 'Visible', 'off')
...
% name_plot=[name_embed,'_',char(roi_name),'_',char(roi_name)];
...
input_save_fig.suffix_plot            = [char(roi_name),'_',char(roi_name)];
...
%   name_plot=fullfile(plot_dir,['erp_curve_',char(roi_name)]);
%             saveas(fig, [name_plot,'.fig']);
...
%     print(fig,fullfile(plot_dir,'erp_curve.ps'),'-append','-zbuffer','-r200')
% % print(fig,fullfile(plot_dir,'erp_curve.ps'),'-append')
%     
% %     str=['ps2pdf ',fullfile(plot_dir,'erp_curve.ps'), ' ',fullfile(plot_dir,'erp_curve.pdf' )];    
% %     system(str);
% end
%  
% 
