...
if tlf1 < 2 || tlf2 < 2
    ...
    for nlf=1:tlf
        mm=erp_plot_mat(nlf,:);
        plot(times, mm,'col',list_col(nlf),'LineWidth',10,'LineStyle',list_stiles{ns});... plot(times, mm,'col',list_col(nlf+1,:),'LineWidth',10,'LineStyle',list_stiles{ns})
        hold on
    ...
    if strcmp(compact_display_stats,'on')
        ...
        if ~isempty(ss)
            ss2=times(ss);
            hold on
            plot(ss2,ones(1,length(ss2))*(yl1(1)+delta/2),'s','LineWidth',10,'MarkerEdgeColor','black','MarkerSize', 5,'MarkerFaceColor', 'black')
        end
    end
...
if tlf1 > 1 && tlf2 > 1
    ...
    for nlf1=1:tlf1
        ...
        for nlf2=1:tlf2
            ...
            plot(times, mm,'col',list_col(nlf2),'LineWidth',10,'LineStyle',list_stiles{ns});...plot(times, mm,'col',list_col(nlf2+1,:),'LineWidth',10,'LineStyle',list_stiles{ns})
            hold on
            if ns < length(list_stiles)
                ns=ns+1;
            else
    ...
    for nlf2=1:tlf2
        ...
        for nlf1=1:tlf1
            ...
            plot(times, mm,'col',list_col(nlf1),'LineWidth',10,'LineStyle',list_stiles{ns});...plot(times, mm,'col',list_col(nlf1+1,:),'LineWidth',10,'LineStyle',list_stiles{ns})
            hold on
            if ns < length(list_stiles)
                ns=ns+1;
            else
