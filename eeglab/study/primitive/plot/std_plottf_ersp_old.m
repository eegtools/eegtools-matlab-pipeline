...
function [pgroup, pcond, pinter] = std_plottf_ersp(timevals, freqs, data, plot_dir,roi_name, name_f1, name_f2, levels_f1,levels_f2,pmaskcond, pmaskgru, pmaskinter,ersp_mode,varargin)
...
file_fig=['ersp_tf_',char(roi_name),'.fig'];
saveas(fig, fullfile(plot_dir,file_fig));

file_tiff=['ersp_tf_',char(roi_name),'.tif'];
print(fig,fullfile(plot_dir,file_tiff),'-dtiff')


os = system_dependent('getos');
if ~ strncmp(os,'Linux',2) 
    print(fig,fullfile(plot_dir,'ersp_tf.ps'),'-append','-dwinc')
    saveppt2(fullfile(plot_dir,'ersp_tf.ppt'),'f',fig);
   
else
    set(gcf,'Renderer','painters')
    print(fig,fullfile(plot_dir,'ersp_tf.ps'),'-append','-zbuffer','-r250')
    
    str=['ps2pdf ',fullfile(plot_dir,'ersp_tf.ps'), ' ',fullfile(plot_dir,'ersp_tf.pdf' )];    
    system(str);
end
 

% ps2pdf('psfile',fullfile(plot_dir,'ersp_tf.ps'),'pdffile',fullfile(plot_dir,'ersp_tf.pdf'))
