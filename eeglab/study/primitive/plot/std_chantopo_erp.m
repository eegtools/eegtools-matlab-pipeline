            ...
            topoplot( pgroupplot{c}, opt.chanlocs, opt.ptopoopt{:});
             ...
             if ~isempty(pmaskgru)
                 if sum(sum(pmaskgru{c}))>0
                    topoplot_masked(pmaskgru{c}, opt.chanlocs, 'style','map','pmask',pmaskgru{c},'threshold',0.5);
                 else
                    topoplot_masked(pmaskgru{c}, opt.chanlocs,'style','blank')
                 end
                 caxis([-1000 1]);    
             else
                 topoplot(pgroupplot{c}, opt.chanlocs, opt.ptopoopt{:});
    ...
    if ~isempty(opt.condstats) && nc > 1
        ...
        if ~isempty(pmaskcond)
            if sum(sum(pmaskcond{g}))>0
                topoplot_masked(pmaskcond{g}, opt.chanlocs, 'style','map','pmask',pmaskcond{g},'threshold',0.5);
            else
                topoplot_masked(pmaskcond{g}, opt.chanlocs,'style','blank')
            end
            caxis([-1000 1]);    
        else
...
if ~isempty(opt.condstats) && ~isempty(opt.groupstats) && ng > 1 && nc > 1
    ...
    if ~isempty(pmaskinter)
        if sum(sum(pmaskinter{3}))>0
               topoplot_masked(pmaskinter{3}, opt.chanlocs,'style','map','pmask',pmaskinter{3},'threshold',0.5);
        else
            topoplot_masked(pmaskinter{3}, opt.chanlocs,'style','blank')
        end
        caxis([-1000 1]);
    else
...
for c = 1:size(hdl,1)
    ...
    if ~mask_surface
        if tlf1 > 1 && tlf2 <= 1
            suptitle([ 'ERP in the ' ,time_window_name,' time-window ','[',num2str(time_window),']: ',  name_f1 ]);
        end

        if tlf1 <=1 && tlf2 > 1
            suptitle([ 'ERP in the ' ,time_window_name,' time-window ','[',num2str(time_window),']: ',   name_f2 ]);
        end
        if tlf1 > 1 && tlf2 > 1 
            suptitle([ 'ERP in the ' ,time_window_name,' time-window ','[',num2str(time_window),']: ',  name_f1 ' and ' name_f2 ]);
        end 
    else
        if tlf1 > 1 && tlf2 <= 1
            suptitle([ 'ERP in the ' ,time_window_name,' time-window ','[',num2str(time_window),']: ',  name_f1]);
...
% name_embed=fullfile(plot_dir,'erp_topo'); 
% name_plot=[name_embed,'_', time_window_name];
...
% name_plot=fullfile(plot_dir,['erp_topo_',time_window_name]);
...
% os = system_dependent('getos');
% if ~ strncmp(os,'Linux',2) 
%     print(fig,fullfile(plot_dir,'erp_topo.ps'),'-append','-dwinc')
%     saveppt2(fullfile(plot_dir,'erp_topo.ppt'),'f',fig);
%    
% else
% %     set(gcf,'Renderer','painters')
%     print(fig,fullfile(plot_dir,'erp_topo.ps'),'-append','-zbuffer','-r200')
% % print(fig,fullfile(plot_dir,'erp_topo.ps'),'-append')
%     
% %     str=['ps2pdf ',fullfile(plot_dir,'erp_topo.ps'), ' ',fullfile(plot_dir,'erp_topo.pdf' )];    
% %     system(str);
% end
%  
% 
