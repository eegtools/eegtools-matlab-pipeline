function [] = std_plotcurve_ersp_tw_fb_compact(times, ersp_curve_fb, plot_dir, roi_name, study_ls, frequency_band_name, time_windows_design_names, name_f1, name_f2, levels_f1,levels_f2, ...
                                                pgroup,  pcond, compact_display_h0,compact_display_v0,compact_display_sem,compact_display_stats,compact_display_xlim,compact_display_ylim,...
                                                ersp_mode)      
        ...
        if strcmp(compact_display_stats,'on')
            ...
            ss=p_plot{1}<study_ls;
             if ~isempty(ss)
                 ss2=times(ss);
                 hold on
                 plot(ss2,ones(1,length(ss2))*(yl1(1)+delta/2),'*','col','black','MarkerSize', 15,'LineWidth',2)
