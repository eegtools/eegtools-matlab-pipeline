% topoplot_masked() - plot a topographic map of a scalp data field in a 2-D circular view 
...
% Usage:
%        >>  topoplot_masked(datavector, EEG.chanlocs);   % plot a map using an EEG chanlocs structure
%        >>  topoplot_masked(datavector, 'my_chan.locs'); % read a channel locations file and plot a map
%        >>  topoplot_masked('example');                  % give an example of an electrode location file
%        >>  [h grid_or_val plotrad_or_grid, xmesh, ymesh]= ...
%                           topoplot_masked(datavector, chan_locs, 'Input1','Value1', ...);
% Required Inputs:
...
%   chan_locs         - name of an EEG electrode position file (>> topoplot_masked example).
%                       Else, an EEG.chanlocs structure (>> help readlocs or >> topoplot_masked example)
% Optional inputs:
...
%                        >> topoplot_masked(values,'chanlocs','plotgrid',[11 12 0; 13 14 15]);
...
%                       See >> topoplot_masked example. If plotrad > 0.5, chans with arc_length > 0.5 
...
%                       see >> topoplot_masked 'example' {default: 'off'}
...
%   'pmask'           - values for masking topoplot_masked. Array of zeros and 1 of the same size as the input 
%                       value array {default: []}
...
%
% Chan_locs format:
%    See >> topoplot_masked 'example'
%
% Examples:
%
%    To plot channel locations only:
%    >> figure; topoplot_masked([],EEG.chanlocs,'style','blank','electrodes','labelpoint','chaninfo',EEG.chaninfo);
%    
...
%        - topoplot_maskeds may be rotated. From the commandline >> view([deg 90]) {default: [0 90])
%
...
%   -now each topoplot_masked axes use the caxis command again.
...
function [handle,Zi,grid,Xi,Yi] = topoplot_masked(Values,loc_file,varargin)

%
%%%%%%%%%%%%%%%%%%%%%%%% Set defaults %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
icadefs                 % read defaults MAXtopoplot_maskedCHANS and DEFAULT_ELOC and BACKCOLOR
...
squeezefac = 1.0;
MINPLOTRAD = 0.15;      % can't make a topoplot_masked with smaller plotrad (contours fail)
VERBOSE = 'off';
MASKSURF = 'off';
...
%
if nargin< 1
   help topoplot_masked;
   return
end

% calling topoplot_masked from Fieldtrip
% -------------------------------
fieldtrip = 0;
...
if fieldtrip
    disp('Calling topoplot_masked from Fieldtrip');
    dir1 = which('topoplot_masked');           dir1 = fileparts(dir1);
    dir2 = which('electrodenormalize'); dir2 = fileparts(dir2);
    addpath(dir2);
    try,
        topoplot_masked(Values, loc_file, varargin{:});
    catch, 
    end;
    addpath(dir1);
    return;
  ...
  if isstr(Values)
    if any(strcmp(lower(Values),{'example','demo'}))
      fprintf(['This is an example of an electrode location file,\n',...
               ...
               '13               180    .181       Pz  \n\n',...
                                                             ...
               'In topoplot_masked() coordinates, 0 deg. points to the nose, positive\n',...
...
if nargs < 2
  ...
  if ~exist(loc_file)
      fprintf('default locations file "%s" not found - specify chan_locs in topoplot_masked() call.\n',loc_file)
      error(' ')
  end
end
if isempty(loc_file)
...
if strcmp(plotgrid,'on')
   ...
   %      fprintf('topoplot_masked() warning: ''plotgrid'' channel matrix has duplicate channels\n');
   % end

   if ~isempty(plotchans)
     if intersect(gchans,abs(plotchans))
        fprintf('topoplot_masked() warning: ''plotgrid'' and ''plotchans'' have channels in common\n');
     end
   end
end
...
if isempty(headrad)  % never set -> defaults
  if plotrad >= rmax
     ...
     if strcmpi(VERBOSE, 'on')
       fprintf('topoplot_masked(): not plotting cartoon head since plotrad (%5.4g) < 0.5\n',...
                                                                    plotrad);
     end
  end
...
% fprintf('topoplot_masked(): plotting %d channels\n',length(pltchans));
...
if ~strcmpi(STYLE,'blank') % if draw interpolated scalp map
 if ~strcmpi(STYLE,'grid') %  not a rectangular channel grid
  ...
  % CLA MASK SOTTO TRESHOLD
  
  Zi(Zi<=threshold)=NaN;
  ZiC(ZiC<=threshold)=NaN;
  ...
  %
   if strcmpi(noplot, 'on') 
    if strcmpi(VERBOSE,'on')
       fprintf('topoplot_masked(): no plot requested.\n')
    end
    return;
   end
  %
