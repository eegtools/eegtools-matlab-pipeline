...
function [STUDY, EEG] = proj_eeglab_study_plot_ersp_tf_fb_baseline(project, analysis_name,  varargin)
...
for design_num=design_num_vec
    ...
    plot_dir=fullfile(results_path,analysis_name,[STUDY.design(design_num).name,'-ersp_tf_roi_baseline','-',str]);
    mkdir(plot_dir);
...
for nroi = 1:length(roi_list)
    ...
    [STUDY ersp_tf times freqs]=std_erspplot(STUDY,ALLEEG,'channels',roi_channels,'noplot','on');
...
%     
    ...
    for nf1=1:tlf1
        ...
        for nf2=1:tlf2
            ...
            tfmean_baseline_rep = repmat(tfmean_baseline,1,length(times),1);
...
%                         tf_plot = (squeeze(mean(tf_mat_norm,3)))';
            ...
            subplot(1,2,2);
            imagesc(times,freqs,tf_plot_masked);
            set(gca,'YDir','normal');
            ylabel('Freq (Hz)');
            caxis(set_caxis);
            line('XData', [0 0], 'YData', [1 max(freqs)], 'LineStyle', '--','LineWidth', 2, 'Color','k');
