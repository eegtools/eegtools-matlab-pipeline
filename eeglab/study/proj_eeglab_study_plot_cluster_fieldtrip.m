%% [STUDY, EEG] = proj_eeglab_study_plot_cluster_fieldtrip(project, analysis_name, mode, varargin)
%
...
function [STUDY, EEG] = proj_eeglab_study_plot_cluster_fieldtrip(project, analysis_name,  varargin)
...
%     erp_curve_allch_stat.allch_names       = allch_names;
    ...
    STUDY = pop_erpparams(STUDY, 'topotime',[] ,'plotgroups','apart' ,'plotconditions','apart','averagechan','off');
    STUDY = pop_statparams(STUDY, 'mode','fieldtrip','fieldtripmethod','montecarlo','fieldtripmcorrect','cluster','fieldtripalpha',nan);
    ...
    if length(levels_f2)>0;
        STUDY = pop_statparams(STUDY, 'condstats','on','groupstats','off');
        [STUDY, erp_curve_allch, times, ~, pcond, ~]=std_erpplot(STUDY,ALLEEG,'channels',allch,'noplot','on');
    end
    
    if length(levels_f1)>0;
        STUDY = pop_statparams(STUDY, 'condstats','off','groupstats','on');
        [STUDY, ~, times, pgroup, ~, ~]=std_erpplot(STUDY,ALLEEG,'channels',allch,'noplot','on');
...
%     erp_curve_roi_stat.times                 = times;
    ...
    erp_curve_allch_stat.pcond_corr                                     = pcond;
    erp_curve_allch_stat.pgroup_corr                                    = pgroup;
