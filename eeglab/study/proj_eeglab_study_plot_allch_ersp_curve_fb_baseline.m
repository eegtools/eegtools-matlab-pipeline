...
function [STUDY, EEG] = proj_eeglab_study_plot_allch_ersp_curve_fb_baseline(project, analysis_name,  varargin)
...
if not(isempty(project.postprocess.ersp_alch.roi_list))
    allch = project.postprocess.ersp_alch.roi_list;
    tch=length(allch);
    sel_cheeg = 1:tch;
end
...
for nband=1:length(frequency_bands_list)
    band_name = frequency_bands_names{nband};
    vec_band = cell2mat(project.postprocess.ersp.frequency_bands_list(nband));
    condiz = sum(vec_band >= min(freqs) & vec_band <= max(freqs)) == 2;
    ...
    for nf1=1:tlf1
        ...
        for nf2=1:tlf2
            ...
            tf_mat = ersp_curve_allch_fb{nf1,nf2};
            
            sel_t_baseline = ...
                times >= project.epoching.bc_st.ms & ...
                times <= project.epoching.bc_end.ms;
            tf_mat_baseline = tf_mat(sel_t_baseline,:,:);
            tfmean_baseline = mean(tf_mat_baseline,1);
            tfmean_baseline_rep = repmat(tfmean_baseline,length(times),1,1);
            cell_comparison = {tf_mat,tfmean_baseline_rep};
            [t df pvals surog] = statcond(cell_comparison, 'method', stat_method, 'naccu', num_permutations);
...
%             
            ...
            subplot(1,2,2);
            imagesc(times,1:tch,tf_plot_masked);
            set(gca,'YDir','normal');
            set(gca,'YTick',1:tch,'YTicklabel',allch,'FontSize', 8)
            ylabel('');
            caxis(set_caxis);
...
%             end
            ...
            input_save_fig.name_embed = ['ersp_tf_baseline_allch'];
