%% [STUDY, EEG] = proj_eeglab_study_plot_roi_ersp_curve(project, analysis_name, mode, varargin)
%
% calculate and display ersp time series for groups of scalp channels consdered as regions of interests (ROI) in
...
% * project.results_display.ersp.time_smoothing;
...
function [STUDY, EEG] = proj_eeglab_study_ersp_define_roi_tw_datadriven(project, analysis_name,  varargin)

if nargin < 1
    help proj_eeglab_study_plot_roi_ersp_curve;
    return;
end;
...
% data_driven_path = fullfile(results_path,'data_driven','ersp');
...
amplim        = project.results_display.ersp.compact_display_ylim;
select_tw_des_plot_ga_plot=project.postprocess.datadriven.ersp.ga.select_tw_des_plot;
select_tw_des_plot_gf_plot=project.postprocess.datadriven.ersp.gf.select_tw_des_plot;
...
recompute_precompute                   = project.postprocess.datadriven.ersp.recompute_precompute;

recompute_ga                = project.postprocess.datadriven.ersp.ga.recompute;
levels_f1_ga             = project.postprocess.datadriven.ersp.ga.levels_f1;
levels_f2_ga             = project.postprocess.datadriven.ersp.ga.levels_f2;


recompute_gf                = project.postprocess.datadriven.ersp.gf.recompute;
levels_f1_gf             = project.postprocess.datadriven.ersp.gf.levels_f1;
levels_f2_gf             = project.postprocess.datadriven.ersp.gf.levels_f2;

select_tw_des_stat = project.postprocess.datadriven.ersp.select_tw_des_stat;
...
for design_num=design_num_vec
    ...
    ersp_curve_roi_stat.study_des       = STUDY.design(design_num);
    ersp_curve_roi_stat.study_des.num   = design_num;
    %     ersp_curve_roi_stat.roi_names       = roi_names;
    ...
    if isempty(project.postprocess.datadriven.ersp.precompute_folder) || not(exist(project.postprocess.datadriven.ersp.precompute_folder))
        ...
        plot_dir                           = fullfile(results_path,analysis_name,[STUDY.design(design_num).name,'-datadriven_ersp','-',str]);
        mkdir(plot_dir);
    else
    ...
    precompute_file = fullfile(results_path ,['precompute_ersp_allch_', char(STUDY.design(design_num).name), '.mat']);
    exist_precompute = exist(precompute_file);
    
    if (strcmp(recompute_precompute, 'on')  || not(exist_precompute))
        
        % ersp_curve_allch cell array of dimension tlf1 x tlf2 , each cell of
        % dimension freqs x times x channels x subjects
        [STUDY, ersp_curve_allch, times, freqs]=std_erspplot(STUDY,ALLEEG,'channels',allch,'noplot','on');
        ...
        if strcmp(ersp_measure, 'Pfu')
            for nf1=1:length(levels_f1)
                for nf2=1:length(levels_f2)
                    ersp_curve_allch{nf1,nf2}=(10.^(ersp_curve_allch{nf1,nf2}/10)-1)*100;
        ...
        out_precompute.ersp_curve_allch = ersp_curve_allch;
        out_precompute.times = times;
        out_precompute.freqs = freqs;
...
%         out_precompute =1
        ...
        ersp_curve_allch =  out_precompute.ersp_curve_allch;
        times = out_precompute.times;
        freqs = out_precompute.freqs;
    ...
    %                     return;
    %                 end
    %                 %                     ersp_curve_roi{nf1,nf2}=ersp_curve_roi{nf1,nf2}(:,vec_select_subjects);
    %                 ersp_curve_allch{nf1,nf2}=ersp_curve_allch{nf1,nf2}(:,1:length(allch),vec_select_subjects);
    %                 ersp_curve_allch_collsub{nf1,nf2} = mean(ersp_curve_allch{nf1,nf2},3)';
    ...
    for nf1=1:length(levels_f1)
        for nf2=1:length(levels_f2)
            if ~isempty(list_select_subjects)
                vec_select_subjects=ismember(list_design_subjects{nf1,nf2},list_select_subjects);
                ersp_curve_allch_allfreq{nf1,nf2}=ersp_curve_allch{nf1,nf2}(:,:,:,vec_select_subjects);
                ersp_curve_allch_collsub_allfreq{nf1,nf2} = squeeze(mean(ersp_curve_allch{nf1,nf2},4));
                list_design_subjects{nf1,nf2}=list_design_subjects{nf1,nf2}(vec_select_subjects);
    ...
    for nband=1:length(frequency_bands_list)
        ...
        fmin =  project.postprocess.ersp.frequency_bands_list{nband}(1);
        fmax =  project.postprocess.ersp.frequency_bands_list{nband}(2);
        ersp_curve_allch = {};
        ersp_curve_allch_collsub = {};
        
        for nf1=1:length(levels_f1)
            for nf2=1:length(levels_f2)
                sel_freqs = freqs >= fmin & freqs <= fmax;
                ersp_curve_allch{nf1,nf2}=squeeze(mean(ersp_curve_allch_allfreq{nf1,nf2}(sel_freqs,:,:,:),1));
                ersp_curve_allch_collsub{nf1,nf2}=squeeze(mean(ersp_curve_allch_collsub_allfreq{nf1,nf2}(sel_freqs,:,:,:),1))';
        ...
        ga_file = fullfile(ga_mat_dir,['ga_ersp_allch_',frequency_band_name,'_', char(STUDY.design(design_num).name), '.mat']);
        ...
        if (strcmp(recompute_ga, 'on')  || not(exist_grand_average))
            ...
            ersp_curve_allch_ga = ersp_curve_allch(sel_levels_f1_ga_dd,sel_levels_f2_ga_dd);
            
            %         medie per ogni livello di gf e cf (grouping e comparing factor)
            %         in cui i soggetti sono collassati
            
            
            ersp_curve_allch_collsub_ga = ersp_curve_allch_collsub(sel_levels_f1_ga_dd,sel_levels_f2_ga_dd);
            
            dim_ersp_curve_allch = size(ersp_curve_allch_ga{1});
            
            
            %% calcolo medione
            %             dim_ersp_curve_allch = ndims(ersp_curve_allch_ga{1});
            %             collapsing_dimension = dim_ersp_curve_allch+1;
            %
            %             % collapse the cells into a matrix, along an additional dimesion
            %
            %             mat_collapsed_cell = cat(collapsing_dimension, ersp_curve_allch_ga{:});
            %
            ...
            num_current_cells = numel(ersp_curve_allch_ga);
            
            sum_collapsed_cell_all_sub = zeros(dim_ersp_curve_allch(1:2));
            
            
            for nc = 1:num_current_cells
                sum_collapsed_cell_all_sub = sum_collapsed_cell_all_sub + squeeze(mean(ersp_curve_allch_ga{nc},3));
            end
            ...
            input_dd.min_duration                                                               = project.postprocess.ersp.design(design_num).min_duration ;                      % ha senso settare una diversa lunghezza minima a seconda della banda o del tipo del sgnale?
        ...
        if strcmp(do_plots,'on')
            ...
            plot_dir_fb = fullfile(data_driven_path,'ga','plot',frequency_band_name);
            mkdir(plot_dir_fb);
            
            input_ga.ersp_grand_average = mean_collapsed_cell_all_sub{:};
            input_ga.p_grand_average  = output_dd_ga.sigcell_pruned_gf{:};
            input_ga.ersp_avgsub = ersp_curve_allch_collsub_ga;
            ...
            input_ga.plot_dir = plot_dir_fb;
            input_ga.ersp_measure = ersp_measure;
            input_ga.select_tw_des=select_tw_des_plot_ga_plot{design_num};
            
            
            
            eeglab_study_allch_ersp_time_dd_grand_average_graph(input_ga);
        end
        ...
        %     soggetti e i livelli dei fattori da considerare MA non ha fatto
        %     nessuna media
        input_mask.curve_allch = ersp_curve_allch;
        %
        %
        %      struttura in cui ho le tw per ogni canale
        ...
        mask_cell_p_ga_file = fullfile(data_driven_path,'ga','mat',['mask_cell_p_ga_',frequency_band_name,'_', char(STUDY.design(design_num).name), '.mat']);
        ...
        output_mask_cell_p_ga.frequency_band_name = frequency_band_name;
        ...
        out_dir = fullfile(data_driven_path,'ga','text',frequency_band_name);
        if not(exist(out_dir))
            mkdir(out_dir);
        end
        [dataexpcols, dataexp] = text_export_allch_ersp_time_dd_ga(out_dir,output_mask_cell_p_ga);
        ...
        %input_ga_exp.ersp_avgsub = ersp_curve_allch_collsub_ga;
        %input_ga_exp.levels_f1 = levels_f1(sel_levels_f1_ga_dd);
        %input_ga_exp.levels_f2 = levels_f2 (sel_levels_f2_ga_dd);
        %input_ga_exp.ersp_curve_allch = ersp_curve_allch;
        
        %text_export_allch_ersp_time_dd_grand_average(input_ga_exp);
        ...
        if (isempty(name_gf) || isempty(name_cf))
            ...
            gf_file = fullfile(gf_mat_dir,['gf_ersp_allch_',frequency_band_name,'_', char(STUDY.design(design_num).name), '.mat']);
            ...
            if (strcmp(recompute_gf, 'on')  || not(exist_grouping_factor))
                ...
                for nl_gf = 1:tl_gf
                    ...
                    if strcmp(name_gf, name_f1)
                        ...
                        current_cells = ersp_curve_allch(nl_gf,:);
                    else
                        current_cells = ersp_curve_allch(:,nl_gf);
                    end
            ...
            if strcmp(do_plots,'on')
                ...
                plot_dir_fb = fullfile(data_driven_path,'gf','plot',frequency_band_name);
                mkdir(plot_dir_fb);
                
                %         input_gf.ersp_grand_average = mean_collapsed_cell_all_sub{:};
                input_gf.p_gf  = output_dd_gf.sigcell_pruned_gf;
                input_gf.ersp_avgsub = ersp_curve_allch_collsub;
                input_gf.ersp_gf    = cell_gf;
                ...
                input_gf.select_tw_des=select_tw_des_plot_gf_plot{design_num};
                input_gf.ersp_measure = ersp_measure;
                
                
                
%                 eeglab_study_allch_ersp_time_dd_grouping_factor_graph(input_gf);
            ...
            mask_cell_p_gf_file = fullfile(data_driven_path,'gf','mat',['mask_cell_p_gf_',frequency_band_name,'_', char(STUDY.design(design_num).name), '.mat']);
            ...
            output_mask_cell_p_gf.name_gf = name_gf;
            output_mask_cell_p_gf.name_cf = name_cf;
            output_mask_cell_p_gf.frequency_band_name=frequency_band_name;
            ...
            out_dir = fullfile(data_driven_path,'gf','text',name_gf,frequency_band_name);
            if not(exist(out_dir))
                mkdir(out_dir);
            end
            [dataexpcols, dataexp] = text_export_allch_ersp_time_dd_gf(out_dir,output_mask_cell_p_gf);
