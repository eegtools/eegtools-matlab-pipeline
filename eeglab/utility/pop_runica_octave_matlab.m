% pop_runica_octave_matlab() - Run an ICA decomposition of an EEG dataset using runica(), 
%                binica(), or another ICA or other linear decomposition. 
% Usage:
%   >> OUT_EEG = pop_runica_octave_matlab( EEG ); % pops-up a data entry window
%   >> OUT_EEG = pop_runica_octave_matlab( EEG, 'key', 'val' ); % no pop_up
%
% Graphic interface:
...
function [ALLEEG, com] = pop_runica_octave_matlab( ALLEEG, varargin )

com = '';
if nargin < 1   
    help pop_runica_octave_matlab;
    return;
end;
...
allalgs   = { 'runica' 'binica' 'jader' 'jadeop' 'jade_td_p' 'MatlabshibbsR' 'fastica' ...
              ...
              'acsobiro' 'acrsobibpf' 'pearson_ica' 'egld_ica' 'eeA' 'tfbss' 'icaML' 'icaMS' 'cudaica' }; % do not use egld_ica => too slow
selectalg = {};
linenb    = 1;
count     = 1;
...
if nargin < 2 | selectamica
    ...
    result       = inputgui( 'geometry', geometry, 'uilist', promptstr, ...
                             'helpcom', 'pophelp(''pop_runica_octave_matlab'')', ...
                             'title', 'Run ICA decomposition -- pop_runica_octave_matlab()', 'userdata', { alllabels alltypes } );
...
[ g addoptions ] = finputcheck( options, { 'icatype'        'string'  allalgs   'runica'; ...
                            ...
                            'pop_runica_octave_matlab', 'ignore');
...
elseif length(ALLEEG) > 1 & ~strcmpi(g.concatenate, 'on') & ~strcmpi(g.concatcond, 'on')
    [ ALLEEG com ] = eeg_eval( 'pop_runica_octave_matlab', ALLEEG, 'warning', 'off', 'params', ...
...
elseif length(ALLEEG) > 1 & strcmpi(g.concatcond, 'on')
    ...
    for index = 1:length(dats)
        ALLEEG(dats{index}) = pop_runica_octave_matlab(ALLEEG(dats{index}), 'icatype', g.icatype, ...
    ...
    com = sprintf('%s = pop_runica_octave_matlab(%s, %s);', inputname(1),inputname(1), ...
...
% 22-6-2017 CLAUDIO CAMPUS & DAVIDE QUARONA
     ...
     case 'cudaica'
        ...
        if isOctave
            icadefs;
            fprintf(['Warning: If the binary CUDAICA function does not work, check that you have added the\n' ...
                     'binary file location (in the EEGLAB directory) to your Unix /bin directory (.cshrc file)\n']);
            if exist(CUDAICABINARY) ~= 2
                error('Pop_runica(): binary CUDAICAICA executable not found. Edit icadefs.m file to specify the CUDAICABINARY location');
            end;
            tmprank = getrank(tmpdata(:,1:min(3000, size(tmpdata,2))));
            if tmprank == size(tmpdata,1) | pca_opt
                [EEG.icaweights,EEG.icasphere] = cudaica_octave( tmpdata, 'lrate', 0.001, g.options{:} );
            else 
                disp(['Data rank (' int2str(tmprank) ') is smaller than the number of channels (' int2str(size(tmpdata,1)) ').']);
                [EEG.icaweights,EEG.icasphere] = cudaica_octave( tmpdata, 'lrate', 0.001, 'pca', tmprank, g.options{:} );
            end;
        else            
            icadefs;
            fprintf(['Warning: If the binary CUDAICA function does not work, check that you have added the\n' ...
            ...
            if tmprank == size(tmpdata,1) | pca_opt
                ...
                disp(['Data rank (' int2str(tmprank) ') is smaller than the number of channels (' int2str(size(tmpdata,1)) ').']);
                [EEG.icaweights,EEG.icasphere] = cudaica( tmpdata, 'lrate', 0.001, 'pca', tmprank, g.options{:} );
            end;
        end
...
if nargin < 2 || selectamica
    com = sprintf('%s = pop_runica_octave_matlab(%s, %s);', inputname(1), inputname(1),  vararg2str(g.options) ); %vararg2str({ 'icatype' g.icatype 'dataset' g.dataset 'options' g.options }) );
end;
