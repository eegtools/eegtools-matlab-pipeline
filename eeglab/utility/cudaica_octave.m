% cudaica_octave() - Run stand-alone binary version of runica() from the
...
% Usage:
%  >> [wts,sph] = cudaica_octave( datavar,  'key1', arg1, 'key2', arg2 ...);
% else
%  >> [wts,sph] = cudaica_octave('datafile', chans, frames, 'key1', arg1, ...);
%
% Inputs:
...
function [wts,sph,tmpint] = cudaica_octave(data,var2,var3,var4,var5,var6,var7,var8,var9,var10,var11,var12,var13,var14,var15,var16,var17,var18,var19,var20,var21,var22,var23,var24,var25)
...
flags{1}='DataFile';args{1}=datafile;
flags{2}='WeightsOutFile';args{2}=weightsfile;
flags{3}='SphereFile';args{3}=spherefile;
flags{4}='chans';args{4}=int2str(nchans);
flags{5}='frames';args{5}=int2str(nframes);
...
%    eval_call = ['!' CUDAICABINARY ' -f ' pwd '/' scriptfile];
%    eval(eval_call);
eval_call = ['system(''' CUDAICABINARY ' -f ' pwd '/' scriptfile ''')']
...
str=['system(''ls -l', scriptfile,' ', weightsfile,' ',spherefile, ''')'];
eval(str)
