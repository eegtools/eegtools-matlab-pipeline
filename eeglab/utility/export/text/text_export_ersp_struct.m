...
if (length(ersp_struct.study_des.variable) > 1)
    ...
    for nroi=1:length(ersp_struct.roi_names) % for each roi
        for nband=1:length(ersp_struct.frequency_bands_names) % for each frequency band
            for nl1=1:length(ersp_struct.study_des.variable(1).value) % for each level of factor 1
                for ntw=1:length(ersp_struct.group_time_windows_names_design) % for each time window
                    roi=[roi; repmat(ersp_struct.roi_names(nroi),length(ersp_struct.list_design_subjects{nl1,1}),1)];
                    band=[band; repmat(ersp_struct.frequency_bands_names(nband),length(ersp_struct.list_design_subjects{nl1,1}),1)];
                    f1=[f1; repmat(ersp_struct.study_des.variable(1).value(nl1),length(ersp_struct.list_design_subjects{nl1,1}),1)];
                    tw=[tw; repmat(ersp_struct.group_time_windows_names_design(ntw),length(ersp_struct.list_design_subjects{nl1,1}),1)];
                    
                    subj=[subj; ersp_struct.list_design_subjects{nl1,1}'];
                    
                    erspmean=[erspmean; ersp_struct.dataroi(nroi).databand(nband).datatw.find_extrema.curve{nl1,1}(ntw,:)'];
                    ersp_extr=[ersp_extr; ersp_struct.dataroi(nroi).databand(nband).datatw.find_extrema.extr{nl1,1}(ntw,:)'];
                    ersp_extr_lat=[ersp_extr_lat; ersp_struct.dataroi(nroi).databand(nband).datatw.find_extrema.extr_lat{nl1,1}(ntw,:)'];
                end
            end
        end
    end
