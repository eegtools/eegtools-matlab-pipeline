...
function [dataexpcols, dataexp] = text_export_erp_onset_offset_sub_continuous_struct(out_file,erp_struct)
...
if (length(erp_struct.study_des.variable) > 1)
        ...
        for nl1=1:length(erp_struct.study_des.variable(1).value) % for each level of factor 1
            for nl2=1:length(erp_struct.study_des.variable(2).value) % for each level of factor 2
                ...
                for nsub=1:length(sub_list) % for each subject
                    data = erp_struct.dataroi(nroi).datatw.onset_offset.sub{nl1,nl2,nsub}.continuous;
    ...
    if isempty(char(erp_struct.study_des.variable(1).label))
        dataexpcols={ char(erp_struct.study_des.variable(2).label) ,  'roi', 'sub',...
        ...
        dataexp=[cell_f2,                    cell_roi, cell_sub,...
    ...
    elseif isempty(char(erp_struct.study_des.variable(2).label))
        ...
        dataexpcols={ char(erp_struct.study_des.variable(1).label) , 'roi', 'sub',...
            'times',   'curve',                    'pvec_raw',    'pvec_corrected',                  'sigvec', ...
            };
        ...
        dataexp=[cell_f1,                    cell_roi, cell_sub,...
            cell_times, cell_curve,                 cell_pvec_raw,    cell_pvec_corrected,              cell_sigvec,...
            ];
        
    else
        dataexpcols={ char(erp_struct.study_des.variable(1).label), char(erp_struct.study_des.variable(2).label),'roi', 'sub',...
            'times',    'curve',                    'pvec_raw',    'pvec_corrected',                  'sigvec', ...
            };
        ...
        dataexp=[                      cell_f1,                      cell_f2,                       cell_roi, cell_sub,...
            cell_times, cell_curve,           cell_pvec_raw,                cell_pvec_corrected,           cell_sigvec,...
            ];
    end
    ...
    for nroi=1:length(erp_struct.roi_names) % for each roi
        for nl1=1:length(erp_struct.study_des.variable(1).value) % for each level of factor 1
            ...
            for nsub=1:length(sub_list) % for each subject
                data = erp_struct.dataroi(nroi).datatw.onset_offset.sub{nl1,nsub}.continuous;
