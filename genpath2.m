function p = genpath2(d)
%   modified version of GENPATH Generate recursive toolbox path.


%   DOES NOT ADD : '.svn' and '.ignore' folders
...
for i=1:length(dirs)
   ...
   if    ~strcmp( dirname, '.')          && ...
         ...
         ~strcmp( dirname, '.svn') && ...         
         ~strcmp( dirname, '.test') && ...         
         ~strcmp( dirname, '.next_version') && ...         
         ~strcmp( dirname, '.ignore')          
      p = [p genpath2(fullfile(d,dirname))]; % recursive calling of this function.
   end
end
