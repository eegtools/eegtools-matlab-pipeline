...
GEVtotal = sum((SpatCorr(sub2ind(size(SpatCorr),K,size(L,1))).*squeeze(std(X)).^2) ./sum(squeeze(std(X)).^2));
...
%     GFP = GFP';
% end
GFP_trial = reshape(GFP,Ntrials,size(L,2)); 
%% For each MS class...
for k = 1:K
    for trial = 1:Ntrials
        
        % Mean GFP
        MGFP(trial,k) = nanmean(GFP_trial(trial,L(trial,:)==k));
        ...
        GEV(trial,k) = sum( (GFP_trial(trial,L(trial,:)==k) .* MspatCorrTMP(k,L(trial,:)==k)).^2) ./ sum(GFP_trial(trial,:).^2);
