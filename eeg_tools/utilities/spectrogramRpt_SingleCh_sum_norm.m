function h0 = spectrogramRpt_SingleCh_sum_norm(EEG,ch, stageStats)
...
if ~isempty(stageStats)
    delta = (delta./nansum(delta( intersect(stageStats.milestones(1,1):stageStats.milestones(2,1), find(and(stages>1,stages<5))) )))*100;
    legendText = ('% Whole Night NREM');
else
    delta = (delta./nansum(delta))*100;
...
if ~isempty(stageStats)
    ...
    for cy=1:size(cycles,1)
        ...
        deltaCy(cy)=nansum(delta(intersect(find(and(stages>1,stages<5)),cycleBegin:cycleEnd)));
...
if ~isempty(stageStats)
    sigma = (sigma./nansum(sigma( intersect(stageStats.milestones(1,1):stageStats.milestones(2,1), find(and(stages>1,stages<5))) )))*100;
    legendText = ('% Whole Night NREM');
else
    sigma = (sigma./nansum(sigma))*100;
    legendText = ('% Whole Night');
end
...
if ~isempty(stageStats)
    ...
    for cy=1:size(cycles,1)
        ...
        sigmaCy(cy)=nansum(sigma(intersect(find(and(stages>1,stages<5)),cycleBegin:cycleEnd)));
        mid(cy)=median(cycleBegin:cycleEnd);
        line([cycleBegin/(60/stageStats.stageData.win)/60 cycleBegin/(60/stageStats.stageData.win)/60],ylim,'Color','green','LineWidth',.75,'LineStyle','--');
        line([cycleEnd/(60/stageStats.stageData.win)/60 cycleEnd/(60/stageStats.stageData.win)/60],ylim,'Color','red','LineWidth',.75,'LineStyle','--');
    end
