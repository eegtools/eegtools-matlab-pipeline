  ...
  case 'drawp' % Redraw EEG and change position
    ...
    for fi=1:tot_marks;
        ...
        if isempty(findobj(gcf,'tag',['tagtext_' g.chan_marks_struct(fi).label]));
            text(double(figdim(1)-(.01*figdim(2)-figdim(1))*fi),double(ylims(2)-0.005),['> ',g.chan_marks_struct(mark_inds(fi)).label], ...
                'color',plot_colour, ...
                'interpreter','none', ...
...
%cc
tmp_color=repmat(g.color,1,round(length(chan_inds)/length(g.color)));
tmp_color = tmp_color(1:length(chan_inds));
...
line(1:length(low_high_range), ...
            data(chan_inds(ci),low_high_range) + tmp_offset, zeros(1,length(low_high_range))+1, ...
            'color', tmp_color{ci}, 'clipping','on','tag',['line_' num2str(chan_inds(ci))]);
    end
    ...
    for tmi=1:length(g.time_marks_struct);
        ...
        for i=1:length(time_marks_offset)
            ...
            if isempty(findobj(gcf,'tag',['text_' g.time_marks_struct(tmi).label]));
                text(double(figdim(2)), ...
                    double(ylims(1)+(inter_time_mark_offset*.5)+inter_time_mark_offset*tmi+(time_marks_offset(i))-(inter_time_mark_offset*length(g.time_marks_struct))), ...
    ...
    colormap(cmap);
    %cc 
    ch_char = flip(g.ax1.YTickLabel);
    tch_char = size(ch_char,1)-1; 
    cell_ch=cell(tch_char,1);
    ind_ch=cell(tch_char,1);
    for nch_char = 1:tch_char
        cch = ch_char(nch_char,:);
        cch=cch(not(ismember(cch,' ')));
        cell_ch{nch_char} = cch;
        ind_ch{nch_char} = nch_char;
    end
    merged_ch = [cell_ch,ind_ch]';
    disp(merged_ch);
  ...
  case 'drawb' % Draw background ******************************************************
    ...
    if strcmpi(g.plotevent, 'on')
        ...
        for index = 1:length(event2plot)
            ...
            try
                tmph2 = text(double([tmplat]), double(ylims(2)-0.005), [EVENTFONT evntxt], ...
                                    'color', g.eventcolors{ event2plot(index) }, ...
                                    'horizontalalignment', 'left',...
                                    'rotation',90);
            catch
