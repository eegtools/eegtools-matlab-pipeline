...
%   avg_func:                       % 1: Absolute value of average: abs(mean(x))
                                    ...
                                    % AGGIUNGO ANCHE LA DISTINZIONE DEI
                                    % GRUPPI
%                                    
function sFiles = brainstorm_group_stats_baseline_ttest_new(protocol_name, cond,  analysis_type, group_name, prestim, poststim, avg_func, subjects_list, varargin)
    ...
    sFiles, [], ...
    'tag', [analysis_type, '_', group_name,'_', cond],  ...
    'output', 1);  % Add to comment    
    
    src_filename    = fullfile(brainstorm_data_path, sFiles(1).FileName);
    dest_filename   = fullfile(brainstorm_data_path, '@inter', ['presults_', cond,'_', group_name '_vs_baseline_', analysis_type, '.mat']);
