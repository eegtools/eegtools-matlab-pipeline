...
function brainstorm_subject_scout_fc_aec1n(protocol_name, result_file, tag,...
    downsample_atlasname, scouts_name, scoutfunc_str, scouttime, freqbands,isorth,...
...
for ntw = 1:length(tw_list)
    ...
    % FileNamesA = bst_process('CallProcess', 'process_plv1n', FileNamesA, [], ...
    %     'timewindow', tw_sec, ...
    %     'scouts',     {downsample_atlasname, scouts_name}, ...
    %     'scoutfunc',  scoutfunc, ...  % 1 =   Mean
    %     'scouttime',  scouttime, ...  % 2 = After
    %     'freqbands',  freqbands,...{'delta', '2, 4', 'mean'; 'theta', '5, 7', 'mean'; 'alpha', '8, 12', 'mean'}, ...
    %     'mirror',     mirror,...0, ...
    %     'keeptime',   keeptime,...1, ...
    %     'plvmeasure', plvmeasure,...2, ...  % 2=Magnitude
    %     'outputmode', outputmode...1 = % Save individual results (one file per input file)
    % );
    
    
    % Process: Amplitude Envelope Correlation NxN
    FileNamesA = bst_process('CallProcess', 'process_aec1n', FileNamesA, [], ...
        'timewindow', tw_sec, ...
        'scouts',     {downsample_atlasname, scouts_name}, ...
        'scoutfunc',  scoutfunc, ...  % 1 =   Mean
        'scouttime',  scouttime, ...  % 2 = After
    ...
    dest                = fullfile(brainstorm_data_path, odir, ['timefreq_connectn_aec' str2, '_', downsample_atlasname, tw_str,  iext]);
