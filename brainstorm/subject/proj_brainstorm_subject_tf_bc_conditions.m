function proj_brainstorm_subject_tf_bc_conditions(project, varargin) ... settings_path, protocol_name, subj_name, level_name,varargin)
...
for subj=1:length(list_select_subjects)
    ...
    for cond=1: project.brainstorm.numcond
        ...
        input_file =  'timefreq_morlet.mat';
        ...
        FileNamesA = bst_process('CallProcess', 'process_baseline_norm', FileNamesA, [], ...
            'baseline',  project.brainstorm.tf.bc.baseline, ...
            'method',    project.brainstorm.tf.bc.method,...'db', ...  % Scale with the mean (dB):    x_std = 10 * log10(x / &mu;)
            'overwrite', 0);
