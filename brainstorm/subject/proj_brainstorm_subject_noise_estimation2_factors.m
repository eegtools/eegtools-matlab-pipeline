...
%%
function proj_brainstorm_subject_noise_estimation2_factors(project, subj_name, varargin)
...
...FileNamesA = cell(1, project.epoching.numcond);
    cnt_trials = 0;
for cond=1:length(project.study.factors)   %project.epoching.numcond
    
    [sStudies, iStudies] = bst_get('StudyWithCondition', [subj_name '/' project.study.factors(cond).level]);
