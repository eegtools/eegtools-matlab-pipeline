...
function ResultFile = brainstorm_results_get_from_subjectslist_by_tag2(data_path,subjects_list, condition_name, data_file, file_tag)

    for s=1:length(subjects_list)
        
        [sStudy, iStudy, iResults] = bst_get('ResultsForDataFile', [subjects_list{s} '/' condition_name '/' data_file]);
        sFiles = {fullfile(data_path, sStudy.Result.FileName)}; 
        sFiles = bst_process(...
