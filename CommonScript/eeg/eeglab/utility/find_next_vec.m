function [out] = find_next_vec(vec1, vec2)
%% function [out] = find_next_vec(vec1, vec2)
...
% out.vec2_next     = vec2_next;                                             % vector. selected elements of vector 2 
% 
% 
...
idx0 = min(mst_dif(:,1:size(mst_dif,2)));
[~,idx] = min(mst_dif(:,1:size(mst_dif,2)));

vec2_next = vec2(idx);

vec2_next(isnan(idx0)) = nan;
