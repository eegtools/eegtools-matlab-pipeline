...
function tfave = tftopo_tw_fb(tfdata,times,freqs,varargin);
...
elseif strcmpi(g.logfreq, 'native'), 
    ...
    if g.denseLogTicks
        minTick = min(ylim);
        maxTick = max(ylim);
        set(gca,'ytick',linspace(minTick, maxTick,50));set(gcf, 'Visible', 'off');
    end;
    ...
    set(gca,'ytick',ftick(inds(1:2:end)));set(gcf, 'Visible', 'off');
    set(gca,'yticklabel', num2str(ft(inds(1:2:end))));set(gcf, 'Visible', 'off');
else
...
xl=xlabel('Time (ms)');
set(xl,'fontsize',12);
set(gca,'yaxislocation','left');set(gcf, 'Visible', 'off');
if g.showchan>0
...
set(gca,'fontsize',12);set(gcf, 'Visible', 'off');
set(gca,'ydir','normal');set(gcf, 'Visible', 'off');
