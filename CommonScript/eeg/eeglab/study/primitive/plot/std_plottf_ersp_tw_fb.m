% std_plottf_ersp_tw_fb() - plot ERSP/ITC images a component
...
function [pgroup, pcond, pinter] = std_plottf_ersp_tw_fb(timevals, freqs, data, time_windows_names, frequency_bands_names, plot_dir,roi_name, name_f1, name_f2, levels_f1,levels_f2,...
                                                        pmaskcond, pmaskgru, pmaskinter,ersp_mode,display_pmode, varargin)
...
for c = 1:nc
    for g = 1:ng
        ...
        if ~isempty(data{c,g})
            ...
            tftopo_tw_fb( tmpplot, timevals, freqs, 'events', tmpevents, 'title', opt.titles{c,g}, options{:});set(gcf, 'Visible', 'off');
...
if ~isempty(opt.groupstats) && ~isempty(opt.condstats) && ng > 1 && nc > 1
    ...
    tftopo_tw_fb( pinterplot,  timevals, freqs, 'title', opt.titles{nc+1,ng+1}, options{:});set(gcf, 'Visible', 'off');
    caxis([-maxplot maxplot]);
    %     ylabel('');
    xlabel('Time window','FontSize',12);
    ylabel('Frequency band','FontSize',12);
