...
try
    ...
    if target_trials ~= baseline_trials
        ...
        disp(target_indices)
        disp(baseline_indices)
              
        if length(target_indices)<length(baseline_indices)
            bad_trigger_index= type_noboudary(find(~ismember(baseline_indices,target_indices)));
        else
            bad_trigger_index= type_noboudary(find(~ismember(target_indices,baseline_indices)));
        end
          disp(bad_trigger_index)
        %                 save('trig_err_idx.mat','fn', 'bad_trigger_index', 'type' ,'-append')
        
        A = {num2str(bad_trigger_index); type};
        
        xlswrite([OUTEEG.filename '_.xlsx'],A)
        OUTEEG = [];
        return;
    end
